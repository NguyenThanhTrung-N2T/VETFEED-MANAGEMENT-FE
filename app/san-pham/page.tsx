  ...
  const filteredProducts = (Array.isArray(products) ? products : []).filter(
    (p) => {
      ...
      const matchesSearch =
        ...
        (p.maSPCode &&
          p.maSPCode.toLowerCase().includes(searchTerm.toLowerCase()));

      // 2. Lọc theo loại sản phẩm (So sánh chính xác mã)
      const matchesCategory =
        activeCategory === "ALL" || p.loaiSanPham === activeCategory;
      ...
      <section className="relative pt-32 pb-20 px-6 bg-[#25396f] overflow-hidden">
        ...
        <div className="container mx-auto text-center relative z-10 space-y-6">
          ...
          <p className="text-blue-100 text-lg max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-5 duration-700 delay-100">
            Cung cấp đầy đủ các loại thuốc thú y và thức ăn chăn nuôi chất lượng
            cao, đảm bảo nguồn gốc xuất xứ rõ ràng.
          </p>
        ...
        {loading ? (
            ...
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <div className="relative aspect-[4/3] bg-slate-50 rounded-xl overflow-hidden mb-4 border border-slate-50">
                  ...
                  {product.loaiSanPham && (
                    <div
                      className={`absolute top-3 left-3 px-2.5 py-1 rounded-lg text-xs font-bold shadow-sm flex items-center gap-1 backdrop-blur-sm
                                            ${
                                              product.loaiSanPham ===
                                              "THUOC_THU_Y"
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  <h3
                    className="text-lg font-bold text-[#1a237e] line-clamp-2 mb-2 group-hover:text-[#d4af37] transition-colors"
                    title={product.tenSP}
                  >
                    {product.tenSP}
                  </h3>
          ...
          <div className="text-center py-24 bg-white rounded-3xl border border-dashed border-slate-200">
            ...
            <p className="text-slate-500 mt-2">
              Vui lòng thử từ khóa hoặc danh mục khác.
            </p>
          </div>
        )}
      </section>

...
const SummaryCard: React.FC<{
  card: { label: string; color: string; maxNumber?: number };
}> = ({ card }) => {
  // Always call the hook unconditionally
  const countValue = useCountUp(card.maxNumber || 0);
  const value = card.maxNumber ? countValue : 0;
  ...
  return (
    ...
    >
      <div
        className={`flex justify-between mb-2 text-${card.color}-900 text-sm font-semibold`}
      >
        <span>{card.label}</span>
      </div>

      <div
        className={`text-4xl font-bold text-${card.color}-900 animate-pulse`}
      >
...
export const OverviewLoading: React.FC<OverviewLoadingProps> = ({
  summaryCards,
  chartBars,
  showPieChart = false,
}) => {
  return (
    <div className="space-y-6">
      {/* SUMMARY CARDS */}
      <div
        className={`grid grid-cols-1 md:grid-cols-${summaryCards.length} gap-6`}
      >
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
          ...
          <div className="h-100 flex items-end justify-between gap-1 md:gap-3 px-2">
            {chartBars.map((height, i) => (
              <div
                key={i}
                className="w-full flex flex-col justify-end h-full gap-2 group"
              >
                <div

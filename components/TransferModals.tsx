...
import { X, Plus, Trash2, Edit, Info, Save, Loader2, Search, AlertTriangle, Filter, RefreshCcw, Package } from "lucide-react";
...
export default function TransferModals({ isOpen, type, selectedId, onClose, onApplyFilter }: TransferModalsProps) {
    ...
    // --- Lọc lô hàng dựa trên Kho Xuất đã chọn ---
    const filteredBatches = useMemo(() => {
        // 1. Nếu chưa chọn kho xuất -> Không có lô
        if (!formData.maKhoXuat) return [];

        // 2. Tìm danh sách tồn kho của kho đã chọn
        const selectedWarehouseInventory = inventory.find(inv => inv.maKho === formData.maKhoXuat);
        const batchesInWarehouse = selectedWarehouseInventory ? selectedWarehouseInventory.danhSachTonKho : [];

        // 3. Lọc theo từ khóa tìm kiếm
        const kw = batchKeyword.trim().toLowerCase();
        if (!kw) return batchesInWarehouse.slice(0, 12);

        const list = batchesInWarehouse
            .filter((b: any) => {
                const code = (b.maPNCode || "").toLowerCase(); // Dùng Mã Phiếu Nhập làm code hiển thị
                const name = (b.tenSP || "").toLowerCase();
                return code.includes(kw) || name.includes(kw);
            })
            .sort((a: any, b: any) => {
                // Ưu tiên khớp tên trước
    ...
    }, [inventory, formData.maKhoXuat, batchKeyword]);
    ...
    const loadMasterData = async () => {
        try {
            const [ws, inv] = await Promise.all([
                ...
                transferService.getInventory()
            ]);
            setWarehouses(ws);
            setInventory(inv);
        } catch (error) {
            console.error("Load master data failed", error);
            setFormError("Không thể tải dữ liệu kho / tồn kho.");
        }
    };
    ...
    // Handle chọn lô từ danh sách tồn kho
    const handleBatchSelect = (b: any) => {
        setNewItem((prev) => ({ ...prev, maLo: b.maLo }));
        // Hiển thị Mã Phiếu Nhập và Tên SP
        setBatchKeyword(`${b.maPNCode || 'N/A'} - ${b.tenSP}`);
        setIsBatchDropdownOpen(false);
        setActiveBatchIndex(0);
        setNewItemError(null);
        markDirty();
    ...
    const handleAddItem = async () => {
        ...
        try {
            ...
            // Tìm thông tin batch trong danh sách tồn kho hiện tại
            const selectedWarehouseInventory = inventory.find(inv => inv.maKho === formData.maKhoXuat);
            const batchInfo = selectedWarehouseInventory?.danhSachTonKho.find((b: any) => b.maLo === newItem.maLo);

            const item: CTChuyenKhoItem = {
                maLo: newItem.maLo,
                maLoCode: batchInfo?.maPNCode || "N/A", // Dùng mã PN làm mã lô hiển thị
    ...
    // ... (Giữ nguyên handleUpdateStatus, handleDelete)
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 text-slate-800">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[600px] animate-in fade-in zoom-in duration-200">
                    <div className="p-8 flex flex-col items-center">
                        ...
                        <h2 className="text-2xl font-bold font-serif mb-8 text-slate-800">Lọc phiếu chuyển</h2>
                        ...
                        <div className="flex gap-4 w-full mt-8">
                            <button onClick={handleFilterSubmit} className="flex-1 py-3 bg-[#43a047] hover:bg-green-700 text-white font-bold rounded-lg transition-colors shadow-lg shadow-green-100">
                                Áp dụng lọc
                            </button>
                            <button onClick={handleResetFilter} className="px-4 py-3 bg-gray-100 hover:bg-gray-200 text-slate-600 font-bold rounded-lg transition-colors" title="Xóa bộ lọc">
                                <RefreshCcw size={20} />
                            </button>
                            <button onClick={() => handleRequestClose()} className="flex-1 py-3 border border-red-500 text-red-600 hover:bg-red-50 font-bold rounded-lg transition-colors">
                                Đóng
                            </button>
                        </div>
                    </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 text-slate-800">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[460px] p-8 animate-in fade-in zoom-in duration-200 text-center text-slate-800">
                    ...
                    <div className="flex gap-4 justify-center mt-6">
                        <button onClick={handleDelete} disabled={isSubmitting} className="flex-1 py-2.5 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 disabled:opacity-60">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-slate-800">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[95vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200 text-slate-800">
                ...
                {isLoading ? (
                    ...
                    <div className="flex-1 overflow-y-auto p-8 bg-[#f8fafc] text-slate-800">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 mb-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-slate-800">
                            {/* Fields: Ngày lập, Ghi chú, Kho nhận */}
                            <div className="space-y-4">
                                <div>
                                    <label className="text-sm font-semibold text-slate-700">Ngày lập phiếu</label>
                                    <input type="date" disabled={isReadOnly} value={formData.ngayLap} onChange={(e) => { setFormData({ ...formData, ngayLap: e.target.value }); markDirty(); }} className="mt-1 w-full p-2.5 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-200 bg-white text-slate-800 disabled:bg-slate-100" />
                                </div>
                                <div>
                                    <label className="text-sm font-semibold text-slate-700">Kho xuất hàng</label>
                                    <select disabled={isReadOnly || isEdit} value={formData.maKhoXuat} onChange={(e) => { setFormData({ ...formData, maKhoXuat: e.target.value }); setNewItem(DEFAULT_NEW_ITEM); setBatchKeyword(""); setIsBatchDropdownOpen(false); setNewItemError(null); markDirty(); }} className="mt-1 w-full p-2.5 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-200 bg-white text-slate-800 disabled:bg-slate-100">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" disabled={isReadOnly} value={formData.ghiChu} onChange={(e) => { setFormData({ ...formData, ghiChu: e.target.value }); markDirty(); }} placeholder="Ví dụ: ưu tiên lô gần hết hạn..." className="mt-1 w-full p-2.5 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-200 bg-white text-slate-800 disabled:bg-slate-100" />
                                </div>
                                <div>
                                    <label className="text-sm font-semibold text-slate-700">Kho nhận hàng</label>
                                    <select disabled={isReadOnly} value={formData.maKhoNhan} onChange={(e) => { setFormData({ ...formData, maKhoNhan: e.target.value }); markDirty(); }} className="mt-1 w-full p-2.5 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-200 bg-white text-slate-800 disabled:bg-slate-100">
                                        <option value="">-- Chọn kho nhận --</option>
                                        {warehouses.map((w) => <option key={w.maKho} value={w.maKho}>{w.tenKho}</option>)}
                                    </select>
                                    {formData.maKhoNhan && formData.maKhoXuat && formData.maKhoNhan === formData.maKhoXuat && <p className="mt-2 text-xs text-red-600 bg-red-50 border border-red-100 rounded-lg p-2">⚠ Kho nhận không được trùng Kho xuất.</p>}
                                </div>
                            </div>
                        </div>
                        ...
                        {!isDetail && (
                            <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 mb-6 shadow-sm text-slate-800">
                                <div className="flex items-start justify-between gap-3 flex-wrap">
                                    ...
                                    <div className="text-xs text-emerald-800/80">{formData.maKhoXuat ? "Gõ mã phiếu nhập / tên SP để tìm nhanh" : "Chọn Kho xuất trước"}</div>
                                </div>

                                <div className="mt-3 grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                                    {/* Batch search */}
                                    <div className="md:col-span-6" ref={batchSearchWrapRef}>
                                        <label className="text-xs font-bold text-emerald-900 mb-1 block">Lô hàng (Tồn kho)</label>
                                        <div className="relative">
                                            <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"><Search size={16} /></div>
                                            <input
                                                disabled={!formData.maKhoXuat}
                                                value={batchKeyword}
                                                onChange={(e) => { setBatchKeyword(e.target.value); setIsBatchDropdownOpen(true); setActiveBatchIndex(0); }}
                                                onFocus={() => { if (formData.maKhoXuat) setIsBatchDropdownOpen(true); }}
                                                onKeyDown={(e) => {
                                                ...
                                                placeholder={formData.maKhoXuat ? "Gõ mã phiếu nhập hoặc tên sản phẩm..." : "Chọn Kho xuất trước"}
                                                ...
                                                <div className="absolute z-20 mt-2 w-full bg-white border border-slate-200 rounded-2xl shadow-lg overflow-hidden text-slate-800">
                                                    <div className="max-h-64 overflow-auto">
                                                        {filteredBatches.length === 0 ? (
                                                            <div className="px-3 py-3 text-sm text-slate-500">Không tìm thấy lô phù hợp trong kho này.</div>
                                                        ) : (
                                                            filteredBatches.map((b: any, idx: number) => (
                                                                <button
                                                                    ...
                                                                    className={["w-full text-left px-3 py-2.5 hover:bg-emerald-50", idx === activeBatchIndex ? "bg-emerald-50" : ""].join(" ")}
                                                                >
                                                                    <div className="flex items-start justify-between gap-2">
                                                                        <div className="min-w-0">
                                                                            <div className="font-medium text-slate-800 truncate">
                                                                                <span className="font-mono text-emerald-700">{b.maPNCode || 'N/A'}</span> <span className="text-slate-400">•</span> {b.tenSP}
                                                                            </div>
                                                                            <div className="text-xs text-slate-500">
                                                                                <Package size={12} className="inline mr-1" />
                                                                                Tồn kho: <span className="font-bold text-slate-700">{b.soLuong}</span>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </button>
                                    ...
                                    <div className="md:col-span-2">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => { setNewItem({ ...newItem, soLuong: Number(e.target.value) }); setNewItemError(null); markDirty(); }}
                                    ...
                                    <div className="md:col-span-3">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => { setNewItem({ ...newItem, ghiChu: e.target.value }); markDirty(); }}
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden text-slate-800">
                            {isEdit && <div className="px-4 py-3 border-b border-slate-100 text-xs text-slate-600 bg-slate-50">✏️ Bạn có thể chỉnh sửa <b>Số lượng</b> khi trạng thái chưa “Đã nhận”.</div>}
                            <div className="overflow-auto">
                                <table className="w-full text-left text-sm">
                                    <thead className="bg-slate-100 text-slate-700 font-bold uppercase text-xs sticky top-0 z-10">
                                        <tr>
                                            <th className="p-3 w-12">#</th>
                                            <th className="p-3 w-40">Mã Lô/PN</th>
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {visibleItems.map((item: any, index: number) => (
                                            <tr key={index} className={["hover:bg-slate-50 transition-colors", flashRowIndex === index ? "bg-emerald-50" : ""].join(" ")}>
                                                ...
                                                <td className="p-3 text-center">
                                                    {isEdit && item.trangThai !== 2 ? (
                                                        <input type="number" min={1} className="w-24 p-2 border border-slate-200 rounded-xl text-center outline-none focus:ring-2 focus:ring-emerald-200 font-bold text-slate-800 bg-white" value={item.soLuongChuyen} onFocus={() => { prevQtyRef.current = Number(item.soLuongChuyen || 0); setFormError(null); }} onChange={(e) => handleQuantityChangeInput(index, e.target.value)} onBlur={(e) => handleQuantityBlur(index, Number(e.target.value))} />
                                                    ) : (
                                                ...
                                                <td className="p-3">
                                                    {isEdit && item.maCTCK && item.trangThai !== 2 ? (
                                                        <select value={item.trangThai ?? 0} onChange={(e) => handleUpdateStatus(item.maCTCK!, Number(e.target.value))} className={["text-xs font-bold py-1.5 px-2.5 rounded-xl border border-slate-200 outline-none cursor-pointer bg-white text-slate-800", item.trangThai === 0 ? "bg-blue-50 text-blue-700 border-blue-100" : item.trangThai === 1 ? "bg-yellow-50 text-yellow-700 border-yellow-100" : "bg-green-50 text-green-700 border-green-100"].join(" ")}>
                                                            <option value={0}>Tạo mới</option>
                                                            <option value={1}>Đang chuyển</option>
                                                            <option value={2}>Đã nhận</option>
                                                        </select>
                                                    ) : (
                                                        <span className={["inline-flex px-2.5 py-1.5 rounded-xl text-xs font-bold border", item.trangThai === 0 ? "bg-blue-50 text-blue-700 border-blue-100" : item.trangThai === 1 ? "bg-yellow-50 text-yellow-700 border-yellow-100" : item.trangThai === 2 ? "bg-green-50 text-green-700 border-green-100" : "bg-gray-50 text-gray-700 border-gray-100"].join(" ")}>
                                                ...
                                                {!isDetail && (
                                                    <td className="p-3 text-center">
                                                        {item.trangThai !== 2 && (
                                                            <button onClick={() => handleRemoveItem(index)} className="inline-flex items-center justify-center w-9 h-9 rounded-xl text-red-500 hover:bg-red-50" title="Xóa dòng">
                                                                <Trash2 size={18} />
                                                            </button>
                                                        )}
                                                    </td>
                                        ...
                                        {visibleItems.length === 0 && <tr><td colSpan={7} className="p-10 text-center text-slate-400">Chưa có sản phẩm nào</td></tr>}
                                    </tbody>
                                </table>
                            </div>
                            <div className="px-4 py-3 border-t border-slate-100 bg-white flex items-center justify-between text-slate-800"><div className="text-xs text-slate-500">Tổng dòng: <span className="font-semibold text-slate-700">{visibleItems.length}</span></div></div>
                        </div>
                    </div>
                )}
                ...
                <div className="p-5 border-t border-gray-100 flex justify-end gap-4 bg-white sticky bottom-0 text-slate-800">
                    {!isDetail && (
                        <button onClick={handleSubmit} disabled={isSubmitting || visibleItems.length === 0} className="px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow-sm flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
